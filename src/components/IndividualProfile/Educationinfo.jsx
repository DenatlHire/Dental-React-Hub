...
class Educationinfo extends Component {
    ...
    getEduPopup(id) {
        ...
        axios.get("university-programs/" + id)
            .then(response => {
                this.setState({
                    isLoaded: true,
    ...
    render() {
        const subTaskEducation = this.state.uniprogramsitems.map((item) => {
            ...
            return (
                <div className="list-group list-group-flush" key={item.id}>
                    <a href className="edit-icon icon-plus shadow" data-toggle="modal" onClick={(e) => this.getEduPopup(item.id, e)} data-target={'#educationinfo' + item.id}><i className="fe fe-edit-2"></i></a>
                    <a className="list-group-item text-reset text-decoration-none" href onClick={this.preventDefault}>
                        {item.course_id.title && (
                            <h2 className="fw-bold mb-1 job_desgnation">
                                <a href data-toggle="modal" onClick={(e) => this.getEduPopup(item.id, e)} data-target={'#educationinfo' + item.id}>{item.course_id.title}</a>
                            </h2>
                        )}
                        {item.university_college_id && (
                            <span className="comp_name">{item.university_college_id.name}</span>
                        )}
                        {item.is_present == '0' ? (
                    ...
                    <div className="modal edit-modal fade" id={'educationinfo' + item.id} data-backdrop="static" data-keyboard="false" tabIndex="-1" role="dialog" aria-labelledby="educationinfoLabel" aria-hidden="true">
                                        ...
                                        <form onSubmit={(event) => this.preventSubmit(event)}>
                                                    ...
                                                    <div className="form-group">
                                                        ...
                                                        <select className="selectoption custom-select form-control" name="university_college_id" onChange={(event) => this.handleUserInput(event)}>
                                                            ...
                                                            {this.state.unicollegeitems.map(uitem => (
                                                                <option value={uitem.id} selected={uitem.id == item.university_college_id.id} key={uitem.id}>{uitem.name}</option>
                                                            ))}
                                                        </select>
                                                        ...
                                                        <select className="selectoption custom-select form-control" name="start_month" onChange={(event) => this.handleUserInput(event)}>
                                                            ...
                                                            {this.state.monthList
                                                                ? this.state.monthList.map((mitem, index) => (
                                                                    <option value={mitem} key={index} selected={mitem == item.start_month} className="li-item">
                                                                        {mitem}
                                                                    </option>
                                                                ))
                                                                : ''}
                                                        ...
                                                        <select className="selectoption custom-select form-control" name="start_year" onChange={(event) => this.handleUserInput(event)}>
                                                            ...
                                                            {this.state.yearList
                                                                ? this.state.yearList.map((yitem, index) => (
                                                                    <option value={yitem} key={index} selected={yitem == item.start_year} className="li-item">
                                                                        {yitem}
                                                                    </option>
                                                                ))
                                                                : ''}
                                            ...
                                            <div className="row">
                                                <div className="col-12 col-md-6">
                                                    <div className="form-group">
                                                        ...
                                                        <input id="is_present" name="is_present" onChange={(e) => this.handleChange(item.id, e)} checked={this.state.checked ? "checked" : ""} type="checkbox" />
                                                        ...
                                                        <select className="selectoption custom-select form-control" name="end_month" onChange={(event) => this.handleUserInput(event)}>
                                                            ...
                                                            {this.state.monthList
                                                                ? this.state.monthList.map((eitem, index) => (
                                                                    <option value={eitem} key={index} selected={eitem == item.end_month} className="li-item">
                                                                        {eitem}
                                                                    </option>
                                                                ))
                                                                : ''}
                                                        ...
                                                        <select className="selectoption custom-select form-control" name="end_year" onChange={(event) => this.handleUserInput(event)}>
                                                            ...
                                                            {this.state.yearList
                                                                ? this.state.yearList.map((eyitem, index) => (
                                                                    <option value={eyitem} key={index} selected={eyitem == item.end_year} className="li-item">
                                                                        {eyitem}
                                                                    </option>
                                                                ))
                                                                : ''}
                                            ...
                                            <div className="row">
                                                <div className="col-12 justify-content-end d-flex">
                                                    <button className="btn btn-primary mb-6 mt-6 mb-md-0 lift" type="submit" onClick={() => this.submit(item.id)}>
                                                        Save
                                                    </button>
        ...
        return (
            <div className="shadow card-border resume-box edu_info border-primary p-5 mt-8 mb-5 bg-white">
                ...
                <a href className="edit-icon shadow" data-toggle="modal" data-target="#educationinfo"><i className="fe fe-plus"></i></a>
