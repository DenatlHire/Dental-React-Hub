...
import { getDesignation, getavailability, getWorkSituation } from "../Service";
...
export default function EditJob() {
  ...
  const getJobDate = () => {
    axios.get(`/clinic-jobs/${id}`).then(response => {
      ...
      setValue(
        "skillset_type_id",
        userInfo?.skills ? userInfo?.skills.split(/\s*,\s*/) : []
      );
      setSelectedhourTime(
        userInfo.availability ? userInfo.availability.split(/\s*,\s*/) : []
      );
      setSelectedhourTimeName(
        userInfo.hours_time_name ? userInfo.hours_time_name.split(/\s*,\s*/) : []
      );
      ...
      setValue(
        `start_date`,
        userInfo.start_date
          ? moment(userInfo.start_date).toISOString()
          : new Date()
      );
      setValue(
        `end_date`,
        userInfo.end_date ? moment(userInfo.end_date).toISOString() : new Date()
      );
                                    ...
                                    <div className="col-md-6">
                                      <div className="f_group p_relative">
                                        ...
                                        <input
                                          ...
                                          defaultValue={currentData.end_date}
                                        />
                                ...
                                <p style={{color: 'red'}} > {errors.address ? errors.address.message : ""} </p>
                                  ...
                                  <div className="row">
                                    <div className="item_form_wrap col-12">
                                      ...
                                      <label>
                                        Enter a specific amount or a range.
                                      </label>
